...
uart_print_str("if cond\n");
	/* Now try to get the SD card's operating condition */
	err = sd_send_op_cond(mmc);
uart_print_str("op cond\n");
	/* If the command timed out, we check for an MMC card */
	if (err == TIMEOUT) {
		err = mmc_send_op_cond(mmc);
uart_print_str("timeout\n");
		if (err) {
			printf("Card did not respond to voltage select!\n\r");
			return UNUSABLE_ERR;
		}
	}
uart_print_str("startup\n");
	err = mmc_startup(mmc);
	if (err)
		mmc->has_init = 0;
	else
...
void print_mmcinfo(struct mmc *mmc)
{
	...
	uart_print_str("Device: ");
	uart_print_str(mmc->name);
	uart_print_str("\n");
	uart_print_str("Manufacturer ID: ");
	uart_print_long( mmc->cid[0] >> 24);
	uart_print_str("\n");
	uart_print_str("OEM:");
	uart_print_long( (mmc->cid[0] >> 8) & 0xffff);
	uart_print_str("\n");
	uart_print_str("Name: ");
	uart_putc( mmc->cid[0] & 0xff);
	uart_putc(mmc->cid[1] >> 24);
	uart_putc((mmc->cid[1] >> 16) & 0xff);
	uart_putc((mmc->cid[1] >> 8) & 0xff);
	uart_putc( mmc->cid[1] & 0xff);
	uart_print_str("\n");
	uart_print_str("Bus width: ");
	uart_print_long(mmc->bus_width);
	uart_print_str("\n");
